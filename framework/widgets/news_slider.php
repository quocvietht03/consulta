<?php
class consulta_News_Slider_Widget extends consulta_Widget {
	function __construct() {
		parent::__construct(
			'consulta_news_slider', // Base ID
			__('News slider', 'consulta'), // Name
				...
				<div class="owl-carousel" data-col_lg="1" data-col_md="1" data-col_sm="1" data-col_xs="1" data-item_space="30" data-loop="true" data-autoplay="false" data-smartspeed="700" data-nav="false" data-dots="true">
					<?php while ($wp_query->have_posts()){ $wp_query->the_post(); ?>
						<article <?php post_class(); ?>>
							...
							<ul class="bt-meta">
								<li><?php _e('By ', 'consulta'); echo get_the_author(); ?></li>
								<li><?php echo get_the_date('M d, Y'); ?></li>
								<li><?php the_terms( get_the_ID(), 'category', __('in ', 'consulta'), ', ' ); ?></li>
							</ul>
							<div class="bt-excerpt"><?php echo consulta_custom_excerpt(21, '.'); ?></div>
						</article>
					<?php } ?>
				</div>
			</div>
...
/* Class consulta_News_Slider_Widget */
function consulta_News_Slider_Widget() {
    register_widget('consulta_news_slider_widget');
}

add_action('widgets_init', 'consulta_news_slider_widget');
