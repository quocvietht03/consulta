<?php
class consulta_Woo_Filter_Attribute extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'consulta' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = __( 'WooCommerce Filter Attribute', 'consulta' );
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'consulta' ),
				'label' => __( 'Title', 'consulta' )
			),
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'consulta' ),
				'options' => $attribute_array
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'consulta' ),
				'options' => array(
					'list'     => __( 'List', 'consulta' ),
					'dropdown' => __( 'Dropdown', 'consulta' ),
					'button' => __( 'Button', 'consulta' )
				)
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'consulta' ),
				'options' => array(
					'and' => __( 'AND', 'consulta' ),
					'or'  => __( 'OR', 'consulta' )
				)
		...
		if ( 0 < count( $terms ) ) {
			...
			if ( 'dropdown' == $display_type ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'consulta' ), wc_attribute_label( $taxonomy ) ) . '</option>';
...
/**
 * Class consulta_Woo_Filter_Attribute
 */
function register_woo_filter_attribute_widget() {
    register_widget('consulta_Woo_Filter_Attribute');
}
