<div class="ro-blog-item">
	<div class="ro-thumb">
		...
		<?php
			$quote_type = get_post_meta(get_the_ID(), 'consulta_post_quote_type', true);
			$quote_content = '';
			if($quote_type == 'custom'){
				$quote_content = get_post_meta(get_the_ID(), 'consulta_post_quote', true);
				$quote_author = get_post_meta(get_the_ID(), 'consulta_post_author', true);
			}
			if ( $quote_content ) {
				?>
				<div class="ro-quote">
	...
	<div class="ro-content">
		...
		<div class="ro-meta">
			...
			<?php if($columns == 1) { ?>
				<a href="<?php the_permalink(); ?>" class="ro-readmore pull-right"><span><?php _e('Read more', 'consulta'); ?></span></a>
			<?php } ?>
		</div>
		<?php } ?>
	</div>
