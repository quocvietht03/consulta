<?php
function consulta_image_carousel_func($atts, $content = null) {
			...
			<div class="owl-carousel" data-col_lg="<?php echo esc_attr($col_lg); ?>" data-col_md="<?php echo esc_attr($col_md); ?>" data-col_sm="<?php echo esc_attr($col_sm); ?>" data-col_xs="<?php echo esc_attr($col_xs); ?>" data-item_space="<?php echo esc_attr($item_space); ?>" data-loop="<?php echo esc_attr($loop); ?>" data-autoplay="<?php echo esc_attr($autoplay); ?>" data-smartspeed="<?php echo esc_attr($smartspeed); ?>" data-nav="<?php echo esc_attr($nav); ?>" data-dots="<?php echo esc_attr($dots); ?>">
				<?php
					foreach($image_ids as $key => $image_id) {
						...
						switch ($click_action) {
							case 'custom_links':
								echo '<a href="'.$image_link.'"><img src="'.$attachment[0].'" alt="'.esc_attr__('thumb', 'consulta').'"/></a>';
								break;
							case 'light_box':
								echo '<a class="lightbox" data-imagelightbox-thumbnail="" href="'.esc_url($full_img[0]).'"><img src="'.$attachment[0].'" alt="'.esc_attr__('thumb', 'consulta').'"/></a>';
								break;
							default:
								echo '<img src="'.$attachment[0].'" alt="'.esc_attr__('thumb', 'consulta').'"/>';
								break;
						}
					}
				?>
...
if(function_exists('bcore_shortcode')) { bcore_shortcode('image_carousel', 'consulta_image_carousel_func');}
